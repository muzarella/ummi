...
$class= $query2->fetch(PDO::FETCH_OBJ) ;
         ...
         echo htmlentities($class->class_code) ; ?> </td>
                     ...
                     <!--    <td>
                         ...
                         <td>
                          <!-- Trigger the modal with a button     onclick="view_student('millz');"    -->
                        <button class="btn btn-info show_modal_result"  data-id=<?php echo htmlentities($student->student_id);?>  data-toggle="modal"  data-target="#add_result"> Edit result  <i class="fa fa-edit" title="add student Result"></i> </button>
                         </td>
                   </tr>
                   <?php 
                   $count = $count +1 ;
